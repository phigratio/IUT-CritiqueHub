...
const serviceAccountKey = JSON.parse(
  fs.readFileSync("./react-js-reviewer-app-firebase-adminsdk-5cort-da9c5350a7.json", "utf8")
);
...
server.get("/get-upload-url", async (req, res) => {
  try {
    ...
    const filename = `${nanoid()}_${dateTime}`; // Generate a unique file name using UUID

    // Create a reference for the file with the current date and time
    const storageRef = ref(storage, `files/${filename}`);

    // Send back the storage reference or download URL so you can use it for uploading
    const metadata = {
      contentType: req.body.contentType || "application/octet-stream", // Accept content type from request
    };
...
server.post("/get-profile", (req, res) => {
  ...
  User.findOne({ "personal_info.username": username })
    .select("-personal_info.password -google-auth -updateAt -blogs")
    .then((user) => {
...
server.post("/create-blog", verifyJWT, (req, res) => {
  ...
  if (!draft) {
    if (!des.length || des.length > 200) {
      return res.status(403).json({
        error: "You must provide blog description within 200 characters",
      });
    }
    if (!banner.length) {
      return res.status(403).json({
    ...
    if (!tags.length || tags.length > 10) {
      return res
        .status(403)
        .json({ error: "Provide tags to publish blog.Maximum 10" });
    }
  }
...
server.post("/get-blog", (req, res) => {
  ...
  Blog.findOneAndUpdate(
    ...
    .then((blog) => {
      ...
      if (blog.draft && !draft) {
        return res.status(500).json({ error: "you can not access drft blogs" });
      }
